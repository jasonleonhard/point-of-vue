...
<script>
  export default {
    ...
    mounted() {
      fetch('https://reddit.com/r/shittyrobots.json')
    ...
    // jump to any route by name (Country, Main, Another, Bla, Img)
      ...
      handleListTap(post) {
        if (post.data.url.includes(".png") || post.data.url.includes(".gif") ||
         post.data.url.includes(".jpg") || post.data.url.includes(".gifv")) {
